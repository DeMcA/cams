...
d3.select(window).on("resize", () => update(selectedData));
...
chart.append("g").attr("class", "x axis upper");
...
function update(data) {
    ...
    if (data.length > 0) {
        chart
            ...
            .attr(
                "transform",
                `translate( ${width / 2}, ${height - yPadding + 35})`
            );
    ...
    var bars = chart.selectAll("rect").data(data, camId);
